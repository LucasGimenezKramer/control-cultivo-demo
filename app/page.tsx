...
export default function Home() {
  ...
  useEffect(() => {
    ...
    mqttClient.on('connect', () => {
      ...
      // NOS SUSCRIBIMOS AL STATUS
      mqttClient.subscribe('planta/status', (err) => {
        if (!err) console.log("Suscrito a planta/status");
      });
    });

    // ESCUCHAR MENSAJES ENTRANTES (Bidireccionalidad)
    mqttClient.on('message', (topic, message) => {
      if (topic === 'planta/status') {
        try {
          ...
          if (data.luces) setChannels(data.luces);
          if (data.horarios) setTimes(data.horarios);
    ...
    <div className="app-container">
      <style jsx global>{`
        * { box-sizing: border-box; }
        ...
        .main-title { font-size: 1.8rem; margin: 0; font-weight: 800; letter-spacing: 1px; background: linear-gradient(to right, #D500F9, #651FFF); -webkit-background-clip: text; color: transparent; }
        ...
        .dashboard-logo { width: 140px; height: auto; display: block; }
        ...
        .menu-btn { width: 100%; padding: 18px 20px; border: none; border-radius: 16px; color: white; font-weight: 700; font-size: 1.2rem; display: flex; align-items: center; justify-content: start; cursor: pointer; box-shadow: 0 4px 10px rgba(0,0,0,0.4); transition: transform 0.1s; }
        ...
        .input-group-large input { width: 100%; padding: 16px; background: #1a1a1a; border: 1px solid #444; border-radius: 10px; color: white; font-size: 1.3rem; margin: 0; }
        input.number-input { padding: 10px 16px; }
        ...
        .save-btn-large { width: 100%; margin-top: 10px; padding: 18px; background: linear-gradient(135deg, #D500F9, #651FFF); border: none; border-radius: 12px; color: white; font-weight: 800; font-size: 1.2rem; letter-spacing: 0.5px; box-shadow: 0 4px 15px rgba(224, 64, 251, 0.3); }
        .toast { position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%); background: #111; border: 1px solid #E040FB; padding: 10px 20px; border-radius: 30px; color: white; font-weight: 600; font-size: 0.95rem; display: flex; align-items: center; white-space: nowrap; z-index: 10000; box-shadow: 0 5px 20px rgba(0,0,0,0.6); animation: popUp 0.3s ease-out; }
